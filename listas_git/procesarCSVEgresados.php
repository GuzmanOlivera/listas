...
function guardarLineaEnBD($stringLinea,$cabecera,$dbh) {
    ...
    $elementos = ['nroCargo','institucionalDSC','lugarResidencia']; //Elementos que no se guardan en la tabla personas pero pueden aparecer en el encabezado
    ...
    if ($posLugarResidencia==-1) {
        $sql = "INSERT INTO egresado(ciEgresado) VALUES($ci)";  //FIXME
    }
    else {
        $lugarResidencia = $lineaArray[$posLugarResidencia];
        $stringResidencia = insertValue($lugarResidencia);
        $sql = "INSERT INTO egresado(ciEgresado,lugarResidencia) VALUES($ci,$stringResidencia)"; 
    }
