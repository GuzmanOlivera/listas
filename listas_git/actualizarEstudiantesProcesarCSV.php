            ...
            switch ($opcionales[$i]) {
                ...
                case 'region':
                    if ($valorElemento!='NULL'){
                        ...
                        $sql = "INSERT INTO region(nombre) VALUES($valorElemento)";                    
                    }
                    echo $sql;    
                    $colField = $dbh->prepare($sql);
                    $colField->execute();  
                    ...
                    $posCarrera = array_search('carrera', $cabeceraArray);
                    $carrera = updateValue($lineaArray[$posCarrera]);
                    
                    $posGeneracion = array_search('generacion', $cabeceraArray);
                    $generacion = updateValue($lineaArray[$posGeneracion]);
                    
                    // Insertar carrera si no existia
                    $sql = "INSERT IGNORE INTO est_carrera(ciEstudianteCarrera,nombreCarrera,generacionCarrera,region) VALUES($ci,$carrera,$generacion,$valorElemento)";
                    echo $sql;    
                    $colField = $dbh->prepare($sql);
                    $colField->execute();
...
// Devuelve la ultima posicion de los elementos de $a2 en $a1
function ultimaPosicion($a1,$a2) {
 $max = -1;
 for($i=0; $i < count($a2); $i++) {
    $posicion = array_search($a2[$i], $a1);  // FIXME: asumimos que el valor de a2 siempre esta en a1. Pero no a la inversa, entonces preguntamos con un if a continuacion
    if($posicion !== FALSE) {
       if ($posicion >= $max) {
          $max = $posicion;
       }
 ...
 *  */ 
    $elementos = ['nroCargo','institucionalDSC','region','carrera','generacion']; //Elementos que no se guardan en la tabla personas pero si en otras
    ...
    // Eliminar carrera del estudiante
    $sql ="DELETE IGNORE FROM est_carrera WHERE ciEstudianteCarrera=$ci";
    ...
    if ($posCarrera==-1) {
      die("Debe haber un campo carrera en la cabecera del archivo");
    }
    $carrera = insertValue($lineaArray[$posCarrera]);
    
    $posGeneracion = buscarPosGeneracion($cabecera);
    if ($posGeneracion==-1) {
      die("Debe haber un campo generacion en la cabecera del archivo");
    }
    $posRegion = buscarPosRegionEstudiante($cabecera);
    if ($posRegion==-1) {
      die("Debe haber un campo region en la cabecera del archivo");
    }
    $region = insertValue($lineaArray[$posRegion]);
    
    $generacion = insertValue($lineaArray[$posGeneracion]);
    
    $sql = "INSERT IGNORE INTO estudiante(ciEstudiante) VALUES($ci)";
    ...
    $sql = "INSERT IGNORE INTO carrera(nombre,generacion) VALUES($carrera,$generacion)";
...
function guardarLineaEnBDMismaPersona($stringLinea,$cabecera,$dbh) {
    ...
    $ci = $lineaArray[$posCI];
     ...
     // Insertamos en tablas relacionadas al estudiante ya ingresado
    $posCarrera = buscarPosCarrera($cabecera);
    if ($posCarrera==-1) {
      die("Debe haber un campo carrera en la cabecera del archivo");
    }
...
//Los cargos que empiezan con 6 son no docente
//stringLinea nunca puede ser vacio cuando se invoca a esta funcion
function guardarLineaEnBD2($stringLinea, $cabecera, $dbh) {
 ...
 * Ademas usaremos la variable $ci para filtrar los datos de manera unica
 *  */ 
    /* Aca hay que separar los campos que son de persona de los que son de estudiante */
    ...
    */ 
    ...
    $stringColValPersona = "";
    $stringColValEstudiante = "";
    
    $arrayCabeceraEstudiante = array('ciEstudiante','generacion','carrera','regionEstudiante'); //FIXME: hacer dinamico
    $arrayCabeceraPersona = array_diff($arrayCabecera,$arrayCabeceraEstudiante);
    
    //$arrayCabecera  //: Contiene los nombres de los campos que vamos a actualizar
            
    $ultimaPosEstudiante = ultimaPosicion($arrayCabecera,$arrayCabeceraEstudiante);
    $ultimaPosPersona = ultimaPosicion($arrayCabeceraPersona,$arrayCabecera);
            
    
    for($i = 0; $i < count($arrayCabecera); $i++) {
       $encontrado = array_search($arrayCabecera[$i], $arrayCabeceraEstudiante); 
       if (!($encontrado!==FALSE)) {
            if ($i == $ultimaPosPersona) {
                if (is_numeric($lineaArray[$i])){
                  $stringColValPersona = $stringColValPersona . $arrayCabecera[$i] . "=" . "$lineaArray[$i]";  
           ...
           if ($i == $ultimaPosEstudiante) {  
                if (is_numeric($lineaArray[$i])){
                  $stringColValEstudiante = $stringColValEstudiante . $arrayCabecera[$i] . "=" . "$lineaArray[$i]";  
    ...
    $stringCondicionEstudiante = "ciEstudiante=" . $ci;
    ...
    $sql = "UPDATE persona SET " . $stringColValPersona . " WHERE " . $stringCondicion . ";" ;
    echo $sql;
    echo "<br>";
    $colField = $dbh->prepare($sql);
    $colField->execute();
    ...
    $sql = "UPDATE estudiante SET " . $stringColValEstudiante . " WHERE " . $stringCondicionEstudiante . ";" ;
    echo $sql;
    echo "<br>";
    $colField = $dbh->prepare($sql);
    $colField->execute();
