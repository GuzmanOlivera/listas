...
function actualizarEnBD($ci,$dbh) { //FIXME: Implementar 
    
    $sql = "UPDATE docente set $atributosValores where ciDocente=$ci";
    $colField = $dbh->prepare($sql);
    $colField->execute();
    
    $sql = "UPDATE egresado set $atributosValores where ciEgresado=$ci";
    $colField = $dbh->prepare($sql);
    $colField->execute();
    
    $sql = "UPDATE estudiante set $atributosValores where ciEstudiante=$ci";
    $colField = $dbh->prepare($sql);
    $colField->execute();
    
    $sql = "UPDATE tas set $atributosValores where ciTAS=$ci";
    $colField = $dbh->prepare($sql);
    $colField->execute();
    
    $sql = "UPDATE personaSinMail set $atributosValores where ci=$ci";
    $colField = $dbh->prepare($sql);
    $colField->execute();
  
    $sql = "UPDATE docente set $atributosValores where ci=$ci";
    $colField = $dbh->prepare($sql);
    $colField->execute();
