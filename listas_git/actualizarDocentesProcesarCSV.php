...
function buscarPosElem($arrayLinea,$elemento) {
   //$arrayLinea = string2array($linea);
   foreach ($arrayLinea as $k => $v)  {
      if ($v == $elemento) {
         return $k;
...
function guardarLineaEnBDMismaPersona($stringLinea,$cabecera,$dbh) {
    ...
    $opcionales = ['aula','region','dptoAcademico','nroCargo','institucionalDSC'];
...
function guardarLineaEnBD($stringLinea,$cabecera,$dbh) {
    ...
    $ci = quitarComillas($lineaArray[$posCI]);

    $elementos = ['nroCargo','institucionalDSC','aula','region','dptoAcademico']; //Elementos que no se guardan en la tabla personas pero pueden aparecer en el encabezado
    ...
    // Insertamos en tabla docente 
    $sql = "INSERT IGNORE INTO docente(ciDocente) VALUES($ci)";  //FIXME
