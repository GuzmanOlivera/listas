 ...
 *  */ 
    ...
    $arrayCabeceraDocente = array('ciDocente','DptoAcademico'); //FIXME: hacer dinamico
    $arrayCabeceraPersona = array_diff($arrayCabecera,$arrayCabeceraDocente);
    
    //$arrayCabecera  //: Contiene los nombres de los campos que vamos a actualizar
            
    $ultimaPosDocente = ultimaPosicion($arrayCabecera,$arrayCabeceraDocente);
    $ultimaPosPersona = ultimaPosicion($arrayCabeceraPersona,$arrayCabecera);
            
    for($i = 0; $i < count($arrayCabecera); $i++) {
       $encontrado = array_search($arrayCabecera[$i], $arrayCabeceraDocente); 
       if (!($encontrado!==FALSE)) {
            if ($i == $ultimaPosPersona) {
                if (is_numeric($lineaArray[$i])){
                  $stringColValPersona = $stringColValPersona . $arrayCabecera[$i] . "=" . "$lineaArray[$i]";  
                    ...
                    if($lineaArray[$i]=='') {
                        $stringColValPersona = $stringColValPersona . $arrayCabecera[$i] . "=NULL";  
                    }
                    else {
                        $stringColValPersona = $stringColValPersona . $arrayCabecera[$i] . "=" . "'" .  "$lineaArray[$i]" . "'";  
                    }
           ...
           if ($i == $ultimaPosDocente) {  
                if (is_numeric($lineaArray[$i])){
                  $stringColValDocente = $stringColValDocente . $arrayCabecera[$i] . "=" . "$lineaArray[$i]";  

                }
                else {
                    if($lineaArray[$i]=='') {
                        $stringColValDocente = $stringColValDocente . $arrayCabecera[$i] . "=" . "NULL";  
                    } else {
                        $stringColValDocente = $stringColValDocente . $arrayCabecera[$i] . "=" . "'" .  "$lineaArray[$i]" . "'";  
    ...
    $stringCondicionDocente = "ciDocente=" . $ci;
    ...
    $sql = "UPDATE docente SET " . $stringColValDocente . " WHERE " . $stringCondicionDocente . ";" ;
    echo $sql;
    echo "<br>";
    $colField = $dbh->prepare($sql);
    $colField->execute();
...
function chequearCabecera($cabecera, $dbh) { //FIXME: Hay que chequear la tabla persona y docente, sumar la cantidad no solapada y verificar que coincida con el numero de la cabecera
