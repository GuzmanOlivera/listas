...
function guardarLineaEnBD($stringLinea,$cabecera,$dbh) {
    ...
    if ($posSeccion==-1) {
        ...
        $seccion = $lineaArray[$posSeccion];
        $stringSeccion = insertValue($seccion);
        $sql = "INSERT IGNORE INTO tas(ciTAS,seccion) VALUES($ci,$stringSeccion)"; 
        
        echo $sql;    
        $colField = $dbh->prepare($sql);
        $colField->execute();  
        
        $sql = "UPDATE tas SET seccion=$stringSeccion WHERE ciTAS=$ci;"; 
